  ...
  return (
    <div className="group bg-white rounded-[26px] p-3 border border-slate-100 shadow-sm transition duration-300 hover:-translate-y-2 hover:shadow-xl hover:border-emerald-100">
      <div className="w-full h-[180px] sm:h-[190px] rounded-[22px] overflow-hidden bg-slate-50">
        <img src={domain + item.img?.url} alt={item.name} className="w-full h-full object-cover transition duration-500 group-hover:scale-105" />
      </div>

      <div className="pt-4 px-1 min-h-[92px] flex flex-col">
        <h3 className="text-sm font-bold text-slate-900 transition duration-300 group-hover:text-emerald-600">{item.name}</h3>

        <p className="text-[11px] text-slate-400 mt-1 whitespace-nowrap overflow-hidden text-ellipsis">{item.desc}</p>
